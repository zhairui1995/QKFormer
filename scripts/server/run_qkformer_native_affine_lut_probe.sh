...
ROOT="${QK_NATIVE_AFFINE_ROOT:-/home/lbz/mac_agent/sdr-lutattn-qkformer-lut}"
PYTHON="${QK_NATIVE_AFFINE_PYTHON:-/home/lbz/miniconda/envs/sdr/bin/python}"
DATA_DIR="${QK_NATIVE_AFFINE_DATA_DIR:-/home/datasets}"
GPU="${1:-0}"
CATEGORY="${2:-all_affine}"
ATTEMPT="${3:-main}"
TIME_STEP="${4:-1}"
MAX_DROP="${QK_NATIVE_AFFINE_MAX_DROP:-1.50}"
CLEAN_TOLERANCE="${QK_NATIVE_AFFINE_CLEAN_TOLERANCE:-0.25}"
...
case "$ATTEMPT" in
  ...
  highres) INTEGER_MAX=31; UNIFORM_BITS=12 ;;
...
"$PYTHON" tools/qkformer_native_affine_lut_probe.py \
  ...
  --layer 4 \
  --time-step "$TIME_STEP" \
  --batch-size "${QK_NATIVE_AFFINE_BATCH_SIZE:-32}" \
  --workers "${QK_NATIVE_AFFINE_WORKERS:-4}" \
  --calib-batches "${QK_NATIVE_AFFINE_CALIB_BATCHES:-32}" \
  --max-eval-batches "${QK_NATIVE_AFFINE_MAX_EVAL_BATCHES:-100000}" \
  --integer-max "$INTEGER_MAX" \
  --uniform-bits "$UNIFORM_BITS" \
  --input-chunk "${QK_NATIVE_AFFINE_INPUT_CHUNK:-4}" \
  --max-drop "$MAX_DROP" \
