...
if ! PYTHON_BIN="$(qk_lut_select_python)"; then
  echo "[qk-lut-e6] missing python3/python"
  exit 1
fi
...
MIN_COUNTS="${QKFORMER_LUT_E6_MIN_COUNTS:-4,8,16,32,64}"
TAG_PREFIX="${QKFORMER_LUT_E6_TAG_PREFIX:-e6_budget}"
ARCHIVE_NAME="${QKFORMER_LUT_E6_ARCHIVE_NAME:-qk_lutformer_e6_budgeted_backoff_artifacts.tar.gz}"

echo "[qk-lut-e6] root=$ROOT"
echo "[qk-lut-e6] commit=$(git rev-parse --short HEAD)"
echo "[qk-lut-e6] checkpoint=$QKFORMER_LUT_CKPT"
echo "[qk-lut-e6] config=$QKFORMER_LUT_E1_CONFIG"
echo "[qk-lut-e6] data_dir=$QKFORMER_LUT_DATA_DIR"
echo "[qk-lut-e6] time_step=$QKFORMER_LUT_TIME_STEP"
echo "[qk-lut-e6] gpu=$QKFORMER_LUT_GPU"
echo "[qk-lut-e6] calib_sizes=$CALIB_SIZES"
echo "[qk-lut-e6] seeds=$SEEDS"
echo "[qk-lut-e6] min_counts=$MIN_COUNTS"
echo "[qk-lut-e6] eval_batches=$QKFORMER_LUT_E1_EVAL_BATCHES"
echo "[qk-lut-e6] tag_prefix=$TAG_PREFIX"
...
for size in "${SIZE_GROUPS[@]}"; do
  ...
  for seed in "${SEED_GROUPS[@]}"; do
    ...
    for min_count in "${MIN_GROUPS[@]}"; do
      ...
      tag="${TAG_PREFIX}_calib${size}_seed${seed}_min${min_count}"
      if ls "results"/qkformer_lut_e1_recon_*_"$tag"/metrics.json >/dev/null 2>&1; then
        echo "[qk-lut-e6] skip_existing tag=$tag"
        continue
      fi
      echo "[qk-lut-e6] run calib_batches=$size seed=$seed min_count=$min_count tag=$tag"
...
if [[ "$TAG_PREFIX" == "e6_budget" ]]; then
  "$PYTHON_BIN" scripts/local/analyze_qk_lut_e6_budgeted_backoff.py
fi

mapfile -t ARTIFACTS < <(
  for d in results/qkformer_lut_e1_recon_*_"$TAG_PREFIX"_calib*_seed*_min*; do
    [[ -d "$d" ]] || continue
...
if [[ "${#ARTIFACTS[@]}" -gt 0 ]]; then
  ...
  echo "[qk-lut-e6] archive=$ROOT/$ARCHIVE_NAME"
fi
echo "[qk-lut-e6] done=$(date -Is)"
