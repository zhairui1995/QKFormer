...
if ! PYTHON_BIN="$(qk_lut_select_python)"; then
  echo "[qk-lut-e2] missing python3/python"
  exit 1
fi

TS="$(date +%Y%m%d_%H%M%S)"
RESULT_DIR="$ROOT/results/qkformer_lut_e2_replace_${TS}"
...
{
  echo "[qk-lut-e2] root=$ROOT"
  echo "[qk-lut-e2] result_dir=$RESULT_DIR"
  echo "[qk-lut-e2] start=$(date -Is)"
  echo "[qk-lut-e2] commit=$(git rev-parse --short HEAD)"
...
if missing:
    for name, exc in missing:
        print(f"[qk-lut-e2] missing dependency: {name}: {exc}")
    print("[qk-lut-e2] run: bash scripts/server/install_qkformer_lut_deps.sh")
    sys.exit(1)

import torch
print(f"[qk-lut-e2] torch={torch.__version__}")
print(f"[qk-lut-e2] cuda_available={torch.cuda.is_available()}")
if not torch.cuda.is_available():
    print("[qk-lut-e2] CUDA is required because upstream QKFormer uses cupy-backed LIF nodes")
    sys.exit(1)
PY
  qk_lut_log_gpu "$PYTHON_BIN" "qk-lut-e2"

  "$PYTHON_BIN" tools/qkformer_lut_e2_replace.py \
...
required = ["classification", "local_replacement", "target_modules"]
missing = [key for key in required if key not in metrics]
if missing:
    raise SystemExit(f"metrics.json missing keys: {missing}")
cls = metrics["classification"]
print(f"[qk-lut-e2] metrics_ok={path}")
print(f"[qk-lut-e2] verdict={metrics.get('verdict')}")
print(f"[qk-lut-e2] baseline_top1={cls['baseline']['top1']}")
print(f"[qk-lut-e2] replacement_top1={cls['replacement']['top1']}")
print(f"[qk-lut-e2] delta_top1={cls['delta']['top1']}")
PY

  echo "[qk-lut-e2] done=$(date -Is)"
} 2>&1 | tee "$LOG_FILE"
