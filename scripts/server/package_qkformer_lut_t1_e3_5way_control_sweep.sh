...
OUT="${1:-qk_lutformer_t1_e3_5way_control_artifacts.tar.gz}"
...
if tar -tzf "$OUT" | grep -E '\.(pth|pth\.tar|pt|ckpt)$' >/dev/null; then
  echo "[qk-lut-package-5way] refusing archive with checkpoint-like file" >&2
  rm -f "$OUT"
  exit 1
fi

if [[ "$OUT" = /* ]]; then
  echo "[qk-lut-package-5way] wrote=$OUT"
else
  echo "[qk-lut-package-5way] wrote=$ROOT/$OUT"
fi
