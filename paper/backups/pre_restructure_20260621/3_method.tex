...
\paragraph{Native learned-operator and LIF-transition LUTs.}
We further evaluate a stricter execution audit that does not merely hook an
operator output. Every frozen Conv1d/Conv2d/Linear module is replaced by a
scalar lookup-and-accumulate module, paired inference BatchNorm parameters are
folded into the corresponding affine table, and each MultiStepLIF state update
is replaced by a finite transition table mapping quantized
$(\mathrm{membrane},\mathrm{input})$ states to the next spike and membrane
state. Integer-valued LIF inputs retain exact integer addresses to avoid
artificial recurrent clipping from decoding integer states through a uniform
floating-point grid. This audit tests complete replacement of the learned
affine, paired BN, and LIF-transition operator classes in QKFormer; it still
leaves nonlearned pooling, residual addition, SSA matrix products/gating, and
global or temporal reductions as ordinary arithmetic.
