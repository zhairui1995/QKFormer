...
Execution is currently blocked by local reachability to server-lbz:
`192.168.70.60:22` times out and ping has 100% packet loss while
`192.168.0/16` routes through the local Wi-Fi gateway rather than the
EasyConnect `utun4` interface. Do not report smoke/full results until the
registered runner executes on the server.
