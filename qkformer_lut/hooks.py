...
ATTENTION_CLASS_NAMES = {"Token_QK_Attention", "Spiking_Self_Attention"}
...
class QKAddressDiagnostic:
    ...
    def _register_hooks(self) -> None:
        ...
        for name, module in modules.items():
            ...
            for child_name in ("q_lif", "k_lif", "v_lif", "attn_lif", "proj_lif"):
                child = getattr(module, child_name, None)
                if child is None:
                    continue
                handle = child.register_forward_hook(
                    self._make_hook(name, child_name.replace("_lif", ""))
                )
